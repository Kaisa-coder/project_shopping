...
export default {
  ...
  methods: {
    ...
    // type ：区分三个元素 + - input  disNUm：区分变换量+1 -1 input 最终的值  cart :变化产品(id)
    // 对此操作进行节流防止用户点击过快 出现bug
    handler: throttle(async function (type, disNum, cart) {
      switch (type) {
        ...
        case 'minus':
          ...
          break
        case 'change':
          // 用户输入的是最终的值，非法的(带有汉字||出现负数)，带给服务器数字为0
          /*    if (isNaN(disNum) || disNum < 1) {
            disNum = 0
          } else {
            // 如果存在小数进行取整，带给服务器是变化的量 输入的量-起始值
